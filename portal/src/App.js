...
import { isNewAccount, showAdmin, showBuyer, showConsignor, showReseller, isAccountSelected } from './store/accounts/UserAccountsSelectors';
...
import { isAppLoading, isAppLoaded, isAppError, isAppAuthenticated } from './store/AppSelectors';
...
class AuthAppBody extends Component {
  ...
  render() {
    return (
      <>
      {
        this.props.isAppLoaded ? (<AppPageSection { ...this.props} />) :
          this.props.auth.token !== null ?
            (this.props.isAppLoading ? <AppLoader /> : this.getUser()) :
            (<HandleLoginRedirect />)
      }
      </>
    )
...
class LoginRedirectToDomain extends Component {
  ...
  render() {
    ...
    return (
      this.state.domain ? <Redirect to={url} /> : null
    )
  }
}
  ...
  render() {
    return (
      <Route path="/">
        <Navbar {...this.props} />
        <div className="body">
            { this.props.isAppError ? (<AppError { ...this.props } />) : 
                this.props.isAccountSelected ? <AppPageRoutes { ...this.props } /> :
    ...
    return (
      <div className="error-container">
        ...
        <div className="error-message">
          There was an error loading the app. Try refreshing the page. If the error persists, <span onClick={this.submitLogout} className="link">click here to logout</span> and then log back in. If the issue persists, contact the administrator at admin@resaleglobal.com.
        </div>
      </div>
    )
  }
